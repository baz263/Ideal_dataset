...
def fbprophet_plot(df):
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df.time, y=df['yhat'], mode='lines', name='yhat'))
    ...
    fig.add_trace(go.Scatter(x=df.time, y=df['electric-combined'], mode='lines', name='electric-combined'))
    return fig
